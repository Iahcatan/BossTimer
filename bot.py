...
NOTICE_BF_PATCH_VERSION = "V106_DASHBOARD_QUEUE_AND_FORM_CLEAR_FIX_2026-09-13"
...
# ==========================================
# 🔔 WEB PUSH / FCM NOTIFICATIONS (Dashboard / Android / Chrome)
# Isolated from Discord command, Boss schedule logic, TTS and Voice.
# ==========================================
...
async def _send_web_push_stage(boss_name: str, stage: str, spawn_time: datetime, notice_minutes: int):
    """Send one dashboard push event through Firebase Cloud Messaging."""
    ...
    try:
        ...
        root = await asyncio.to_thread(lambda: db.reference("web_push_tokens").get() or {})
        tokens = []
        if isinstance(root, dict):
            for uid, uid_data in root.items():
        ...
        for uid, token_key, token, language in tokens:
            try:
                if stage == "spawn":
                    if language == "en":
                        title = f"⚔️ {boss_name} — Spawned!"
                        body = f"Boss {boss_name} has spawned!"
                    elif language == "ko":
                        title = f"⚔️ {boss_name} — 생성 완료!"
                        body = f"보스 {boss_name}이(가) 생성되었습니다!"
                    else:
                        title = f"⚔️ {boss_name} — เกิดแล้ว"
                        body = f"บอส {boss_name} เกิดแล้ว"
                else:
                    if language == "en":
                        title = f"⏳ {boss_name} — Spawning Soon!"
                        body = f"Boss {boss_name} will spawn in {notice_minutes} minutes"
                    elif language == "ko":
                        title = f"⏳ {boss_name} — 생성 임박!"
                        body = f"보스 {boss_name}이(가) {notice_minutes}분 후에 생성됩니다."
                    else:
                        title = f"⏳ {boss_name} — ใกล้เกิดใน {notice_minutes} นาที"
                        body = f"บอส {boss_name} จะเกิดใน {notice_minutes} นาที"
        ...
        print(
            f"🔔 Web Push/FCM result | boss={boss_name} | stage={stage} | recipients={len(tokens)} | sent={success_count} | lang=TH/EN",
            flush=True,
        )
    except Exception as exc:
