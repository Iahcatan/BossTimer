...
NOTICE_BF_PATCH_VERSION = "V126_DISCORD_USER_NOTIFY_PERMISSION_FIX_2026-09-23"
...
@app.route("/api/discord-user-notify", methods=["POST", "OPTIONS"])
def discord_user_notify_api():
    """Save per-dashboard-user Discord DM preferences through Firebase Admin SDK.

    V126 fix: V125 wrote these new fields directly from the browser via the Firebase
    Client SDK. Existing RTDB security rules can legitimately deny those fields,
    producing PERMISSION_DENIED even for an approved user. This endpoint authenticates
    the Firebase ID token and performs the narrowly-scoped write server-side with the
    existing Admin SDK, while preventing non-admin users from editing another account.
    """
    ...
    try:
        ...
        target_ref = db.reference(f"users/{target_uid}")
        target_profile = target_ref.get() or {}
        ...
        print(
            f"✅ Discord per-user notification settings saved via Admin SDK | "
        ...
        print(f"⚠️ /api/discord-user-notify permission denied: {exc}", flush=True)
        return _api_json({"success": False, "error": str(exc)}), 401
    except Exception as exc:
        print(f"❌ /api/discord-user-notify failed: {exc}", flush=True)
        traceback.print_exc()
...
DISCORD_USER_DM_MIN_INTERVAL = max(8.0, float(os.environ.get("DISCORD_USER_DM_MIN_INTERVAL", "10")))
...
async def _load_discord_user_dm_preferences(force: bool = False) -> list[dict]:
    """Load opt-in dashboard user Discord DM preferences with a short Firebase cache."""
    ...
    try:
        data = await asyncio.wait_for(
            asyncio.to_thread(db.reference("users").get),
            timeout=8,
        )
    except Exception as exc:
        print(f"⚠️ โหลด Discord per-user notification preferences ไม่สำเร็จ: {exc!r}", flush=True)
        return []
    ...
    if isinstance(data, dict):
        for uid, profile in data.items():
            ...
            if not discord_user_id:
                continue
            # Prevent duplicate DMs when two dashboard accounts point at the same Discord ID.
...
async def queue_discord_user_boss_notifications(
    ...
    for pref in preferences:
        ...
        user = _resolve_cached_discord_user(discord_user_id)
        if user is None:
            skipped_not_cached += 1
            continue
...
async def flush_pending_discord_user_dm_notifications_once():
    ...
    if user is None:
        ...
        print(
            f"⏭️ Discord per-user DM dropped; user not cached | discord_user_id={item.get('discord_user_id')} | boss={item.get('boss_name')} | stage={item.get('stage')}",
            flush=True,
        )
        return
    ...
    try:
        ...
        result = await guarded_discord_call(
            ...
            context=f"boss-user-dm:{item.get('boss_name')}:{item.get('stage')}:{item.get('discord_user_id')}",
            background=True,
        )
        if result is None:
            return
        ...
        with pending_discord_user_dm_lock:
            if pending_discord_user_dm_notifications and pending_discord_user_dm_notifications[0] is item:
                ...
                pending_discord_user_dm_keys.discard(item.get("key"))
        print(
            f"⚠️ Discord per-user DM unavailable (403) | boss={item.get('boss_name')} | stage={item.get('stage')} | discord_user_id={item.get('discord_user_id')} | drop item | {exc}",
            flush=True,
        )
    except discord.HTTPException as exc:
        if getattr(exc, "status", None) == 429:
