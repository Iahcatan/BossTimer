...
NOTICE_BF_PATCH_VERSION = "V140_BACKGROUND_REST_REENTRY_GUARD_FIX_2026-09-23"
...
async def flush_pending_discord_user_dm_notifications_once():
    """Send at most one per-user DM per worker pass, through the existing Discord REST guard."""
    ...
    try:
        ...
        if getattr(exc, "status", None) == 429:
            # Keep the item queued. The central V124 REST breaker owns Retry-After/recovery.
            print(
                f"⏸️ Discord per-user DM held by REST 429 | boss={item.get('boss_name')} | stage={item.get('stage')} | discord_user_id={item.get('discord_user_id')}",
                flush=True,
            )
            return
