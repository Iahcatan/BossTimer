...
NOTICE_BF_PATCH_VERSION = "V144_AUTOATTENDANCE_DURABLE_DELIVERY_DELETE_ACTOR_LOG_FIX_2026-09-24"
...
discord_background_rest_long_recovery_quiet_seconds = max(60.0, float(os.environ.get("DISCORD_BACKGROUND_REST_LONG_RECOVERY_QUIET", "120")))
        ...
        try:
            ...
            if background:
                ...
                with discord_block_lock:
                    ...
                    next_probe_mono = discord_block_next_probe_mono
                bg_remaining = _background_rest_remaining()
                if discord_background_rest_startup_hold or discord_background_rest_quarantined or bg_remaining > 0 or (temp_restriction and next_probe_mono > now_after_wait) or _discord_rest_rate_limit_remaining() > 0:
...
DISCORD_USER_DM_MIN_INTERVAL = max(8.0, float(os.environ.get("DISCORD_USER_DM_MIN_INTERVAL", "8")))
...
# V144: Auto Attendance panel delivery must survive a long Discord REST restriction.
# The activity itself is already persisted in Firebase; these limits only control how
# long a missing Discord panel is retained for eventual delivery after recovery.
AUTO_ATTENDANCE_PANEL_LATE_DELIVERY_SECONDS = max(3600.0, float(os.environ.get("AUTO_ATTENDANCE_PANEL_LATE_DELIVERY_SECONDS", "21600")))
AUTO_ATTENDANCE_PANEL_RESTORE_WINDOW_SECONDS = max(3600.0, float(os.environ.get("AUTO_ATTENDANCE_PANEL_RESTORE_WINDOW_SECONDS", "86400")))
    ...
    for guild_id_raw, activities in root.items():
        ...
        for activity_id, activity in activities.items():
            ...
            if close_dt:
                ...
                if age_after_close > AUTO_ATTENDANCE_PANEL_RESTORE_WINDOW_SECONDS:
                    continue
            scanned += 1
...
async def _flush_one_autoattendance_panel() -> bool:
    ...
    try:
        ...
        if close_dt and now >= close_dt + timedelta(seconds=AUTO_ATTENDANCE_PANEL_LATE_DELIVERY_SECONDS):
            ...
            print(
                f"⏭️ Auto Attendance panel expired after late-delivery window | activity={item['activity_id']}",
                flush=True,
            )
            return False

        # During an active Discord REST restriction, keep the queue item exactly as-is.
        # Do not do Firebase re-reads or any Discord probe; the central guard already owns recovery.
        ...
        try:
            ...
            await _persist_autoattendance_panel_state(
                ...
                str(item["activity_id"]),
                {
                    "panel_delivery_status": "blocked_429" if status == 429 else "http_error",
...
async def _autoattendance_find_or_create_for_schedule(
    ...
    # This preserves the T-30/open -> spawn/close lifecycle even when REST is cooling down.
