...
NOTICE_BF_PATCH_VERSION = "V91_ATTENDANCE_LONGPOLL_WAITRESS_FIX_2026-09-10-R1"
...
_attendance_cache_lock = threading.RLock()
_attendance_change_condition = threading.Condition(_attendance_cache_lock)
...
def _publish_attendance_snapshot(root, reports, *, force=False):
    ...
    with _attendance_change_condition:
        ...
        _attendance_change_condition.notify_all()
    if changed or force:
        print(
...
def attendance_stream_api():
    """Authenticated long-poll Attendance endpoint.

    Kept at the existing /api/attendance-stream URL for Dashboard compatibility,
    but deliberately does NOT use WSGI streaming/SSE because Waitress rejects
    hop-by-hop connection headers on streamed responses. The request waits for
    the server-side Attendance cache version to change, then returns one JSON
    snapshot. If nothing changes within 45 seconds, it returns changed=False.
    """
    origin = request.headers.get('Origin', '')
    ...
    try:
        ...
        since_version = int(since_raw or 0)
    except (TypeError, ValueError):
        since_version = 0

    with _attendance_change_condition:
        current_version = int(_attendance_cache_version)
        if current_version <= since_version:
            _attendance_change_condition.wait(timeout=45.0)
            current_version = int(_attendance_cache_version)
        cached = _attendance_cache_payload
        if cached is not None:
            cached = json.loads(json.dumps(cached, ensure_ascii=False))

    if cached is None:
        return _api_json({'success': True, 'changed': False, 'attendance_version': current_version,
                          'raid_attendance': {}, 'monthly_reports': {}, 'individual_members': {},
                          'server_time': datetime.now(TZ_THAI).isoformat()})

    cached['changed'] = current_version > since_version
    return _api_json(cached)


@app.route('/api/delete-boss'
