...
async def _send_web_push_stage(boss_name: str, stage: str, spawn_time: datetime, notice_minutes: int):
    ...
    try:
            ...
            for uid, uid_data in root.items():
                ...
                for token_key, item in uid_data.items():
                    if isinstance(item, dict) and item.get("token"):
                        tokens.append((str(uid), str(token_key), str(item["token"])))

        title = f"⚔️ {boss_name} — {'เกิดแล้ว' if stage == 'spawn' else f'ใกล้เกิดใน {notice_minutes} นาที'}"
        body = f"บอส {boss_name} เกิดแล้ว" if stage == "spawn" else f"บอส {boss_name} จะเกิดใน {notice_minutes} นาที"

        success_count = 0
        stale = []
        for uid, token_key, token in tokens:
            try:
        ...
        print(
            f"🔔 Web Push/FCM result | boss={boss_name} | stage={stage} | recipients={len(tokens)} | sent={success_count}",
            flush=True,
        )
    except Exception as exc:
