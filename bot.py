...
NOTICE_BF_PATCH_VERSION = "V158_DISCORD_RESTRICTION_RECOVERY_GUARD_AND_PERSISTENCE_FIX_2026-09-26"
...
# V140: serialize the entire background REST lane and keep a substantially more
# conservative start-to-start spacing. This lane is intentionally independent from
# foreground interactions/commands and Voice/TTS. Discord-provided Retry-After remains
# the authority when Discord actually returns HTTP 429.
discord_background_rest_min_interval = max(60.0, float(os.environ.get("DISCORD_BACKGROUND_REST_MIN_INTERVAL", "60.0")))
discord_background_rest_call_lock = asyncio.Lock()
...
discord_background_rest_recovery_grace_seconds = max(30.0, float(os.environ.get("DISCORD_BACKGROUND_REST_RECOVERY_GRACE", "60")))
# V140: repeated long API restrictions were followed by another background 429 soon
# after recovery. For a server-provided restriction of 15 minutes or longer, keep the
# optional/background REST lane quiet for 30 minutes after the one-shot recovery probe.
...
discord_background_rest_long_recovery_quiet_seconds = max(600.0, float(os.environ.get("DISCORD_BACKGROUND_REST_LONG_RECOVERY_QUIET", "1800")))
...
async def wait_for_discord_rest_clear_confirmed(*, context: str):
    """Wait for a real successful REST recovery confirmation before normal REST resumes.

    V151: once a temporary restriction timer has expired, no ordinary REST caller
    is allowed to become the first request that tests the API. The dedicated
    one-shot recovery probe is the only request permitted in that state.
    This closes the race where startup command verification or a background queue
    could otherwise consume the first post-timer request.
    """
    last_log = 0.0
    while True:
        now_wall = time.time()
        ...
        with discord_block_lock:
            ...
            temp_restriction = bool(discord_block_temp_restriction)
            server_until = float(discord_block_server_until or 0.0)
            next_probe = float(discord_block_next_probe_mono or 0.0)

        if not block_active:
            return True

        timer_expired = (server_until > 0 and now_wall >= server_until)
        timer_unknown_but_local_safe = (
            temp_restriction
            and server_until <= 0
            and next_probe > 0
            and now_mono >= next_probe
        )
        if temp_restriction and (timer_expired or timer_unknown_but_local_safe):
            # The server timer (when supplied) or the local safety timer (when Discord
            # supplied no usable timer) must expire first. Then only the single
            # recovery probe may become the first request.
            await _run_expired_discord_recovery_probe()
            await asyncio.sleep(1.0)
            continue

        remaining = max(0.0, next_probe - now_mono) if next_probe > 0 else 60.0
        current = time.monotonic()
        if current - last_log >= 15.0:
            last_log = current
            print(
                f"⏳ Discord REST clear confirmation waiting | context={context} | "
                f"remaining={remaining:.1f}s | no normal HTTP sent",
                flush=True,
            )
        await asyncio.sleep(min(15.0, max(1.0, remaining)))
...
async def _run_expired_discord_recovery_probe():
    ...
    with discord_block_lock:
        ...
        if not active or not temp_restriction:
            return False
        if server_until > 0 and server_until > now_wall:
            return False
        if next_probe_mono > now_mono:
            return False
...
async def guarded_discord_call(
    ...
    *,
    ...
    # V151: when Discord's server timer has expired, do not let an ordinary REST
    # request become the first post-timer request. Only the dedicated one-shot
    # recovery probe may test/clear the restriction.
    if not recovery_probe:
        ...
        if temp_restriction and (timer_expired or local_timer_expired):
            ...
            _log_background_rest_skip(
                context, remaining, reason="awaiting-recovery-probe"
            ) if _is_background_discord_rest_context(context) else _log_rest_skip(context, remaining)
            return None
    ...
    if background:
        ...
        # V140: serialize background requests independently of foreground interactions.
        # The old V139 timer was reserved before the REST lock, so multiple workers could
        # wake together and still become back-to-back requests. Re-checking after acquiring
        # this dedicated lane lock makes the spacing a real start-to-start guarantee.
        await discord_background_rest_call_lock.acquire()
    ...
    async with discord_rest_guard_lock:
        ...
        if temp_restriction and (timer_expired or local_timer_expired) and not recovery_probe:
            ...
            if background:
                _log_background_rest_skip(context, remaining, reason="awaiting-recovery-probe")
            else:
                _log_rest_skip(context, remaining)
            return None
        if temp_restriction and next_probe_mono > now_mono and not recovery_probe:
        ...
        try:
            ...
            if background:
                ...
                if discord_background_rest_startup_hold or discord_background_rest_quarantined or bg_remaining > 0 or recovery_confirmation_pending or (temp_restriction and next_probe_mono > now_after_wait) or _discord_rest_rate_limit_remaining() > 0:
                    ...
                    return None
                discord_background_rest_next_call_at = now_after_wait + discord_background_rest_min_interval
                _record_background_rest_attempt(context)

            result = await call_factory()
            _clear_discord_rest_backoff_after_success()
            _clear_discord_block_after_success(context=context, recovery_probe=recovery_probe)
            ...
            if background and background_lane_lock_acquired:
                discord_background_rest_call_lock.release()
...
DISCORD_USER_DM_MIN_INTERVAL = max(300.0, float(os.environ.get("DISCORD_USER_DM_MIN_INTERVAL", "300")))
...
async def _expire_stale_voice_confirmation(boss_name: str, data: dict, *, now_ms: int | None = None) -> bool:
    ...
    if age_seconds is None or age_seconds <= VOICE_CONFIRMATION_MAX_AGE_SECONDS:
        return False
    ...
    print(
        ...
        f"request={request_id} | age={_format_duration(age_seconds)} | max_age=30m | no retry",
        flush=True,
    )
    return True
...
def queue_voice_confirmation(boss_name: str, data: dict, source: str = 'unknown', wait: bool = False, timeout: float = 180.0):
    ...
    if request_age is not None and request_age > VOICE_CONFIRMATION_MAX_AGE_SECONDS:
        ...
        print(
            ...
            f"request={request_id} | age={_format_duration(request_age)} | max_age=30m | no retry",
            flush=True,
        )
        return False
...
async def check_bf_notifications():
    global last_bf_notified_hour, last_bf_text_notified_hour, last_bf_voice_success_hour, bf_notify_enabled, bf_voice_diag_last_ts
    if not bf_notify_enabled:
        return

    try:
        now = datetime.now(TZ_THAI)
        # Resolve the nearest BF boundary. An even hour is the BF start; odd hours
        # advance to the next even hour. If the current boundary has already passed,
        # move directly to the next 2-hour BF boundary.
        ...
        # BF warning is strictly pre-start only: from 3 minutes before through the
        # boundary itself. Never send a stale BF warning after BF has started.
        if not (0 < seconds_until_bf <= 180):
            return

        trigger_key = candidate.strftime('%Y-%m-%d-%H')
        next_bf_time = candidate.strftime('%H:%M')
        diag_key = f"window:{trigger_key}"
        now_mono = time.monotonic()
        if now_mono - bf_voice_diag_last_ts.get(diag_key, 0.0) >= 60.0:
            bf_voice_diag_last_ts[diag_key] = now_mono
            print(
                f"⏰ BF WARNING WINDOW | now={now.strftime('%Y-%m-%d %H:%M:%S')} | "
                f"next={candidate.isoformat()} | seconds_until={seconds_until_bf:.1f}",
                flush=True,
            )

        for guild in bot.guilds:
            # Text notification remains one-shot and is independently guarded by the
            # central Discord REST rate-limit/recovery protection. It is sent only in
            # the pre-start warning window, never after BF has started.
            retry_at = bf_text_retry_after_ts.get(trigger_key, 0.0)
            if last_bf_text_notified_hour != trigger_key and time.monotonic() >= retry_at:
                print(f"⏰ BF WARNING TRIGGER | guild={guild.name} | now={now.strftime('%H:%M:%S')} | next={next_bf_time}", flush=True)
                mentions = []
                ...
                if text_channel:
                    ...
                    for lang in enabled:
                        embed.add_field(name={"th":"🇹🇭 ไทย","en":"🇺🇸 English","ko":"🇰🇷 한국어"}[lang], value=body_map[lang], inline=False)
                    # Reserve one attempt first. On 429, do not retry this trigger
                    # automatically; the central Discord REST guard handles the cooldown.
                    ...
                    try:
                        ...
                        if send_result is not None:
                            ...
                            print(f"✅ BF text notification sent | guild={guild.name}", flush=True)
                        else:
            ...
            # BF Voice follows the same occupancy rule as Boss Voice:
            # enter only a configured /setvoice room that currently has at least one
            # human member. When no configured room is occupied, do nothing and wait
            # inside the same pre-start warning window in case a member joins later.
            ...
            voice_diag_key = f"voice:{trigger_key}:{guild.id}"
            now_mono = time.monotonic()
            if now_mono - bf_voice_diag_last_ts.get(voice_diag_key, 0.0) >= 60.0:
                bf_voice_diag_last_ts[voice_diag_key] = now_mono
                print(
                    f"⏰ BF VOICE TARGETS | guild={guild.name} | configured={len(configured)} | "
                    f"occupied={len(occupied)} | pending={len(undelivered)} | "
                    f"voice_success={last_bf_voice_success_hour == trigger_key}",
                    flush=True,
                )

            if not occupied:
                if now_mono - bf_voice_diag_last_ts.get(f"wait:{trigger_key}:{guild.id}", 0.0) >= 60.0:
                    bf_voice_diag_last_ts[f"wait:{trigger_key}:{guild.id}"] = now_mono
                    print(
                        f"⏭️ BF VOICE WAIT | guild={guild.name} | no occupied /setvoice rooms yet | "
                        "no voice connection attempted",
                        flush=True,
                    )
                continue
            if not undelivered:
            ...
            for room in undelivered:
                try:
                    print(
                        ...
                        f"humans={len([m for m in room.members if not m.bot])}",
                        flush=True,
                    )
                    ok = False
                    last_exc = None
                    for attempt in range(1, 4):
                        # Refresh occupancy immediately before each attempt so the bot
                        # never joins a room that has become empty.
                        ...
                        try:
                            ...
                            last_exc = exc
                            print(f"⚠️ BF VOICE RETRY {attempt}/3 | guild={guild.name} | channel={room.name} | {exc}", flush=True)
                            await asyncio.sleep(min(2.0 * attempt, 5.0))
                    results.append(bool(ok))
                    if ok:
                        _mark_bf_voice_room_completed(_bf_voice_room_key(trigger_key, guild.id, room.id))
                    if last_exc and not ok:
                        print(f"❌ BF VOICE ERROR FINAL | guild={guild.name}/{room.name} | {last_exc}", flush=True)
            ...
            print(
                f"✅ BF VOICE COMPLETE | guild={guild.name} | success={success_count}/{len(results)} | "
        ...
        for activity_id, activity in activities.items():
            ...
            if close_dt:
                ...
                if age_after_close > AUTO_ATTENDANCE_PANEL_LATE_DELIVERY_SECONDS:
                    print(
                        f"⏭️ Auto Attendance stale/closed panel not restored | activity={activity_id} | "
                        f"age_after_close={age_after_close:.0f}s | delivery_window="
                        f"{AUTO_ATTENDANCE_PANEL_LATE_DELIVERY_SECONDS:.0f}s",
                        flush=True,
                    )
                    continue
            else:
    ...
    try:
        ...
        if close_dt and now >= close_dt + timedelta(seconds=AUTO_ATTENDANCE_PANEL_LATE_DELIVERY_SECONDS):
            ...
            await _persist_autoattendance_panel_state(
                ...
                {
                    "panel_delivery_status": "missed_window",
                    "panel_delivery_missed_at": _attendance_now_iso(),
                },
            )
            print(
                f"⏭️ Auto Attendance panel missed delivery window | activity={item['activity_id']} | "
                f"status=missed_window | close={close_dt.strftime('%Y-%m-%d %H:%M:%S')}",
                flush=True,
            )
            return False
        ...
        print(
            ...
            f"message={message.id} | role_mentions={mention_count}",
            flush=True,
        )
        return True
    finally:
        ...
        while True:
            ...
            try:
                ...
                if status == 429:
                    ...
                    try:
                        _apply_discord_rest_429(e, context="gateway:startup-429", source="GATEWAY")
