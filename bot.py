...
NOTICE_BF_PATCH_VERSION = "V141_REST_STALE_DM_AUTOATTENDANCE_VOICE_RETRY_FIX_2026-09-24"
...
async def _run_boss_voice_stage(boss_name: str, stage: str, notice_minutes: int, target_guilds):
    ...
    try:
        ...
        if spoken_rooms == 0 and not any_occupied_room:
            if stage == "spawn":
                # Spawn notification remains one-shot when no configured Voice room has people.
                # Do not enter an empty room; this preserves the user's occupancy rule.
                await save_boss_notification_flags(boss_name, voice_spawn_sent=True)
            else:
                # Advance notification stays pending while nobody is in Voice. If a member
                # joins later during the existing advance window, the next scheduler tick can
                # announce normally. No TTS/Voice implementation is changed.
                print(
                    f"⏳ Boss VOICE ADVANCE waiting for occupied configured Voice room | "
                    f"boss={boss_name} | retry_on_next_tick=True | role-tag/text notification only",
                    flush=True,
                )
                return
            print(
...
def _queue_autoattendance_panel(guild: discord.Guild, activity: dict, summary_channel, embed, view) -> bool:
    try:
...
async def _flush_one_autoattendance_panel() -> bool:
    ...
    # Do not deliver a stale open-panel after the scheduled close time.
    close_dt = parse_to_thai_datetime(item.get("close_at"))
    if close_dt and datetime.now(TZ_THAI) >= close_dt:
        with pending_auto_attendance_panel_lock:
            if pending_auto_attendance_panels and pending_auto_attendance_panels[0] is item:
                pending_auto_attendance_panels.popleft()
                pending_auto_attendance_panel_keys.discard(item["key"])
        print(
            f"⏭️ Auto Attendance panel expired before Discord send | activity={item['activity_id']}",
            flush=True,
        )
        return False
    ...
    try:
        ...
        print(
            f"⏸️ Auto Attendance panel held after Discord HTTP error | activity={item['activity_id']} | status={getattr(exc, 'status', None)}",
            flush=True,
        )
        return False
    except Exception as exc:
    ...
    with pending_auto_attendance_panel_lock:
        if pending_auto_attendance_panels and pending_auto_attendance_panels[0] is item:
            ...
            pending_auto_attendance_panel_keys.discard(item["key"])
    try:
        bot.add_view(item["view"], message_id=int(message.id))
    except Exception as exc:
        print(f"⚠️ Auto Attendance add_view failed | {activity_id}: {exc}", flush=True)
    print(
        f"✅ Auto Attendance panel sent | guild={guild.name} | activity={activity_id} | message={message.id}",
        flush=True,
    )
    return True
...
async def _autoattendance_find_or_create_for_schedule(
    ...
    if not activity.get("panel_message_id"):
        ...
        view = RaidAttendanceView(activity_id)
        if not _queue_autoattendance_panel(guild, activity, summary_channel, embed, view):
            return False
