...
NOTICE_BF_PATCH_VERSION = "V153_PENDING_VOICE_CONFIRMATION_EXPIRY_FIX_2026-09-25"
...
async def _expire_stale_voice_confirmation(boss_name: str, data: dict, *, now_ms: int | None = None) -> bool:
    ...
    with schedule_lock:
        ...
        if isinstance(current, dict):
            current_request = str(current.get("confirmationRequestId") or "").strip()
            current_status = str(current.get("confirmationStatus") or "").strip().lower()
            if current_request and current_request != request_id:
                return False
...
async def _voice_confirm_boss_recording(boss_name: str, data: dict):
    ...
    try:
        # V153: close an over-age pending confirmation instead of leaving it pending.
        # Leaving confirmationStatus=pending caused the 15-second retry worker to
        # queue the same expired request forever.
