...
NOTICE_BF_PATCH_VERSION = "V151_DISCORD_REST_RECOVERY_ONLY_PROBE_AND_COMMAND_VERIFY_GUARD_FIX_2026-09-24"
    ...
    try:
        try:
            requested_ms = (
                ...
                else datetime.fromisoformat(str(requested_at).replace("Z", "+00:00")).timestamp() * 1000
            )
    ...
    try:
        ...
        if spoken_rooms == 0 and not any_occupied_room:
            if stage == "spawn":
                ...
                print(
                    f"⏳ Boss VOICE ADVANCE waiting: no occupied configured Voice room | "
                    f"boss={boss_name} | retry_on_next_tick=True",
                    flush=True,
                )
        else:
            current_occupied = []
