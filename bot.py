...
NOTICE_BF_PATCH_VERSION = "V109_IPHONE_WEBPUSH_2026-09-13"
...
def web_push_subscribe_api():
    ...
    try:
        ...
        db.reference(f"web_push_tokens/{uid}/{token_key}").set(record)
        print(f"🔔 Web Push/FCM token registered | uid={uid} | key={token_key}", flush=True)
        return _web_push_cors(jsonify({"success": True, "tokenKey": token_key}))
...
def web_push_standard_subscribe_api():
    ...
    try:
        ...
        print(f"🍎 iPhone Web Push subscription registered | uid={uid} | key={sub_key}", flush=True)
        return _web_push_cors(jsonify({"success": True, "subscriptionKey": sub_key}))
    except PermissionError as exc:
        return _web_push_cors(jsonify({"success": False, "error": str(exc)})), 401
    except Exception as exc:
        print(f"❌ /api/push/web-subscribe failed: {exc}", flush=True)
        return _web_push_cors(jsonify({"success": False, "error": str(exc)})), 500
...
async def _send_web_push_stage(boss_name: str, stage: str, spawn_time: datetime, notice_minutes: int):
    """Send one dashboard push event through FCM and standards-based Web Push."""
    ...
    try:
        sent_record = await asyncio.to_thread(lambda: db.reference(f"web_push_sent_events/{event_key}").get() or {})
        if not isinstance(sent_record, dict):
            sent_record = {}
        fcm_done = bool(sent_record.get("fcmDone"))
        standard_done = bool(sent_record.get("standardWebPushDone"))
        if not fcm_done and int(sent_record.get("successCount") or 0) > 0:
            # V100-V108 event records used successCount for FCM-only delivery.
            fcm_done = True
            ...
            for uid, token_key, token, language in tokens:
                ...
                try:
                    ...
                    if "registration-token-not-registered" in code or "unregistered" in code.lower() or "not a valid fcm registration token" in text:
                        ...
                        print(f"⚠️ Web Push/FCM send failed | boss={boss_name} | stage={stage} | uid={uid} | {exc}", flush=True)
            if fcm_stale:
                for uid, token_key in fcm_stale:
                    try:
                        ...
                        pass
            fcm_done = True if not tokens or fcm_success_count > 0 else False
        ...
        if not standard_done:
            if not subscriptions:
                standard_done = True
            elif not _ios_vapid_public_key_b64url() or not WEB_PUSH_IOS_VAPID_PRIVATE_KEY:
                ...
                for uid, sub_key, subscription, language in subscriptions:
                    ...
                    try:
                        ...
                        if delivered:
                            ...
                            print(f"⚠️ iPhone Web Push send failed | boss={boss_name} | stage={stage} | uid={uid} | status={status} | {detail}", flush=True)
                    except Exception as exc:
                        print(f"⚠️ iPhone Web Push send failed | boss={boss_name} | stage={stage} | uid={uid} | {exc}", flush=True)
                for uid, sub_key in standard_stale:
                    try:
                        await asyncio.to_thread(db.reference(f"web_push_subscriptions/{uid}/{sub_key}").delete)
                    except Exception:
                        pass
                standard_done = True if not subscriptions or standard_success_count > 0 else False

        if fcm_done or standard_done:
            await asyncio.to_thread(
                db.reference(f"web_push_sent_events/{event_key}").set,
                {
                    "bossName": boss_name,
                    "stage": stage,
                    "spawnTime": spawn_time.isoformat(),
                    "sentAt": datetime.now(timezone.utc).isoformat(),
                    "successCount": fcm_success_count if fcm_attempted else int(sent_record.get("successCount") or 0),
                    "fcmDone": fcm_done,
                    "fcmSuccessCount": fcm_success_count if fcm_attempted else int(sent_record.get("fcmSuccessCount") or sent_record.get("successCount") or 0),
                    "standardWebPushDone": standard_done,
                    "standardWebPushSuccessCount": standard_success_count,
                },
            )

        print(
            f"🔔 Web Push result | boss={boss_name} | stage={stage} | fcm={fcm_success_count}/{fcm_attempted} | iphone={standard_success_count}/{standard_attempted} | fcm_done={fcm_done} | iphone_done={standard_done}",
            flush=True,
        )
    except Exception as exc:
