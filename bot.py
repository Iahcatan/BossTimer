...
TRANSLATION_CONFIG_DEFAULT = {
    "enabled": False,
    "mode": "separate_channels",
    "source_channel_id": "",
    "th_channel_id": "",
    "en_channel_id": "",
    "ko_channel_id": "",
}
_translation_config_cache = {}
_translation_cache = {}
_translation_cache_order = deque()
_translation_rate_events = deque()
...
def _translation_config_path(guild_id):
    return f"translation_config/{int(guild_id)}"


def _normalize_translation_config(value):
    src = value if isinstance(value, dict) else {}
    out = dict(TRANSLATION_CONFIG_DEFAULT)
    out["enabled"] = parse_bool(src.get("enabled"), False)
    mode = str(src.get("mode") or "separate_channels").strip().lower()
    out["mode"] = mode if mode in {"separate_channels", "same_channel"} else "separate_channels"
    for key in ("source_channel_id", "th_channel_id", "en_channel_id", "ko_channel_id"):
        raw = str(src.get(key) or "").strip()
        out[key] = raw if raw.isdigit() else ""
    return out


def load_translation_config_for_guild(guild_id):
    key = str(guild_id)
    try:
        value = db.reference(_translation_config_path(guild_id)).get() or {}
        cfg = _normalize_translation_config(value)
        _translation_config_cache[key] = cfg
        return cfg
    except Exception as exc:
        print(f"⚠️ Translation config load failed | guild={guild_id} | {exc}", flush=True)
        return dict(_translation_config_cache.get(key) or TRANSLATION_CONFIG_DEFAULT)


def _store_translation_cache(cache_key, value):
    if cache_key in _translation_cache:
        _translation_cache[cache_key] = value
        return
    _translation_cache[cache_key] = value
    _translation_cache_order.append(cache_key)
    while len(_translation_cache_order) > TRANSLATION_CACHE_SIZE:
        oldest = _translation_cache_order.popleft()
        _translation_cache.pop(oldest, None)


def _detect_translation_source(text):
    sample = re.sub(r"\s+", " ", str(text or "").strip())
    if not sample:
        return None
    thai = len(re.findall(r"[\u0E00-\u0E7F]", sample))
    ko = len(re.findall(r"[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F]", sample))
    latin = len(re.findall(r"[A-Za-z]", sample))
    if thai > 0 and thai >= max(2, ko) and thai >= max(2, latin // 2):
        return "th"
    if ko > 0 and ko >= max(2, thai) and ko >= max(2, latin // 2):
        return "ko"
    if latin > 0:
        return "en"
    return None
...
async def _translation_rate_limit_wait():
    deadline = time.monotonic() + TRANSLATION_RATE_WAIT_MAX_SECONDS
    while True:
        async with _translation_rate_lock:
            now = time.monotonic()
            cutoff = now - 60.0
            while _translation_rate_events and _translation_rate_events[0] <= cutoff:
                _translation_rate_events.popleft()
            if len(_translation_rate_events) < TRANSLATION_MAX_PER_MINUTE:
                _translation_rate_events.append(now)
                return True
            wait_for = max(0.05, 60.0 - (now - _translation_rate_events[0]))
            if now >= deadline:
                return False
        await asyncio.sleep(min(wait_for, 1.0))


async def _get_translation_session():
    global _translation_session
    if _translation_session and not _translation_session.closed:
        return _translation_session
    async with _translation_http_lock:
        if _translation_session and not _translation_session.closed:
            return _translation_session
        ...
        try:
            ...
            async with session.post(endpoint, json=payload) as response:
                ...
                if response.status in {429, 403, 503}:
                    _translation_disabled_until = time.monotonic() + 5.0
                last_error = RuntimeError(f"HTTP {response.status}: {body[:240]}")
        except Exception as exc:
            last_error = exc
            continue
    _translation_stats["failures"] += 1
    raise last_error or RuntimeError("all free translation providers failed")


async def _translate_cached(text, source, target):
    normalized = str(text or "").strip()
    if not normalized:
        return ""
    key = hashlib.sha256(f"{source}|{target}|{normalized}".encode("utf-8")).hexdigest()
    if key in _translation_cache:
        _translation_stats["hits"] += 1
        return _translation_cache[key]
    _translation_stats["misses"] += 1
    translated = await _translate_free_api(normalized, source, target)
    _store_translation_cache(key, translated)
    return translated


def _translation_marker(message_id, target):
    return f"<!-- SKYNET_TRANSLATION message={message_id} lang={target} -->"


def _strip_translation_marker(text):
    return str(text or "").replace("<!-- SKYNET_TRANSLATION", "<!--SKYNET_TRANSLATION")


async def _run_translation_for_message(message):
    ...
    if not content:
        return
    if "SKYNET_TRANSLATION" in content:
        return
    if len(content) > TRANSLATION_MAX_CHARS:
        content = content[:TRANSLATION_MAX_CHARS].rstrip() + " …"

    cfg = _normalize_translation_config(_translation_config_cache.get(str(message.guild.id)) or load_translation_config_for_guild(message.guild.id))
    if not cfg.get("enabled"):
        return
    source_id = str(cfg.get("source_channel_id") or "")
    if source_id != str(message.channel.id):
        return

    source_lang = _detect_translation_source(content)
    if source_lang not in TRANSLATION_LANGS:
        return

    targets = [("th", cfg.get("th_channel_id")), ("en", cfg.get("en_channel_id")), ("ko", cfg.get("ko_channel_id"))]
    used_destinations = set()
    same_channel = str(cfg.get("mode") or "separate_channels") == "same_channel"
    for target_lang, configured_channel_id in targets:
        if target_lang == source_lang:
            continue
        channel_id = source_id if same_channel else str(configured_channel_id or "").strip()
        if not channel_id:
            continue
        if not same_channel and channel_id == source_id:
            continue
        destination_key = (channel_id, target_lang)
        if destination_key in used_destinations:
            continue
        used_destinations.add(destination_key)
        try:
            translated = await _translate_cached(content, source_lang, target_lang)
            if not translated or translated.strip() == content.strip():
                continue
            channel = message.channel if same_channel else message.guild.get_channel(int(channel_id))
            if channel is None:
                try:
            ...
            if channel is None or not hasattr(channel, "send"):
                continue
            author_name = clean_display_name(getattr(message.author, "display_name", getattr(message.author, "name", "User")))
            embed = discord.Embed(description=translated, color=discord.Color.blurple())
            embed.set_author(name=author_name)
            embed.set_footer(text=f"SKYNET Translator • {source_lang.upper()} → {target_lang.upper()}")
...
@app.route("/api/translation/config", methods=["GET", "POST", "OPTIONS"])
def translation_config_api():
    if request.method == "OPTIONS":
        return _web_push_cors(jsonify({"success": True})), 204
    try:
        uid, profile = _verify_dashboard_user_request()
        guild_id = str(os.environ.get("TRANSLATION_GUILD_ID", "759359960009539616")).strip()
        if request.method == "GET":
            cfg = load_translation_config_for_guild(guild_id)
            return _web_push_cors(jsonify({"success": True, "config": cfg}))
        if profile.get("role") != "admin":
            return _web_push_cors(jsonify({"success": False, "error": "Admin only"})), 403
        payload = request.get_json(silent=True) or {}
        cfg = _normalize_translation_config(payload)
        db.reference(_translation_config_path(guild_id)).set(cfg)
        _translation_config_cache[str(guild_id)] = cfg
        print(f"🌐 Translation config updated | guild={guild_id} | enabled={cfg['enabled']} | mode={cfg['mode']} | source={cfg['source_channel_id']}", flush=True)
        return _web_push_cors(jsonify({"success": True, "config": cfg, "updatedBy": uid}))
    except PermissionError as exc:
        return _web_push_cors(jsonify({"success": False, "error": str(exc)})), 401
    except Exception as exc:
        print(f"❌ /api/translation/config failed: {exc}", flush=True)
        return _web_push_cors(jsonify({"success": False, "error": str(exc)})), 500


@app.route("/api/translation/status", methods=["GET", "OPTIONS"])
def translation_status_api():
    if request.method == "OPTIONS":
        return _web_push_cors(jsonify({"success": True})), 204
    try:
        cfg = load_translation_config_for_guild(str(os.environ.get("TRANSLATION_GUILD_ID", "759359960009539616")).strip())
        return _web_push_cors(jsonify({
            "success": True,
            "enabled": bool(cfg.get("enabled")),
            "mode": cfg.get("mode"),
            "provider": "LibreTranslate public mirrors (free/no API key by default)",
            "cacheHits": _translation_stats["hits"],
            "cacheMisses": _translation_stats["misses"],
            "requests": _translation_stats["requests"],
            "failures": _translation_stats["failures"],
            "cacheSize": len(_translation_cache),
            "cacheLimit": TRANSLATION_CACHE_SIZE,
            "rateLimitPerMinute": TRANSLATION_MAX_PER_MINUTE,
...
async def _voice_confirm_boss_recording(boss_name: str, data: dict):
    ...
    try:
        ...
        # V111: If /setvoice is configured but nobody is currently in those rooms,
        # do not enter or wait for Voice. Text notification/role mention remains the
        # notification path; complete the confirmation request for Voice purposes.
        if has_configured_target and not has_occupied_target:
            ...
            print(
                ...
                f"boss={boss_name} | request={request_id} | role-tag/text notification only",
                flush=True,
            )
            try:
                await asyncio.to_thread(
                ...
                print(f"⚠️ Could not persist skipped Voice confirmation for {boss_name}: {exc}")
            return True
...
async def on_message(message: discord.Message):
    # V116 translator is isolated from slash commands and existing bot features.
    try:
        await _run_translation_for_message(message)
    except Exception as exc:
        print(f"⚠️ Translation listener failed safely: {exc}", flush=True)
    # Preserve any existing prefix-command compatibility.
    try:
...
async def on_ready():
    ...
    load_translation_config_for_guild(os.environ.get("TRANSLATION_GUILD_ID", "759359960009539616"))
...
async def _run_boss_voice_stage(boss_name: str, stage: str, notice_minutes: int, target_guilds):
    ...
    try:
        ...
        if spoken_rooms == 0 and not any_occupied_room:
            ...
            print(
                f"⏭️ Boss VOICE {stage.upper()} skipped: no occupied configured Voice room | "
                f"boss={boss_name} | role-tag/text notification only",
                flush=True,
            )
        elif spoken_rooms > 0 and all_ok:
            if stage == "advance":
                await save_boss_notification_flags(boss_name, voice_notice_sent=True)
