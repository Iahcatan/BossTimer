...
NOTICE_BF_PATCH_VERSION = "V157_TIME_INTERACTION_429_NO_REST_FALLBACK_FIX_2026-09-25"
...
async def wait_for_discord_rest_clear_confirmed(*, context: str):
    ...
    while True:
        ...
        if temp_restriction and server_until > 0 and now_wall >= server_until:
            # The timer has expired, but the restriction is not considered clear
            # until the single recovery probe succeeds. The probe itself is guarded
            # by discord_block_recovery_probe_started_at, so this call cannot create
            # a second probe race.
    ...
    *,
    ...
    if not recovery_probe:
        with discord_block_lock:
            ...
            server_until = float(discord_block_server_until or 0.0)
            next_probe_mono = float(discord_block_next_probe_mono or 0.0)
        if temp_restriction and server_until > 0 and time.time() >= server_until:
