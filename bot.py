...
async def _run_translation_for_message(message):
    ...
    for target_lang, channel_id in targets:
        channel_id = str(channel_id or "").strip()
        if not channel_id or target_lang == source_lang or channel_id == source_id or channel_id in used_channel_ids:
            continue
        ...
        try:
            ...
            print(f"⚠️ Translation failed | guild={message.guild.id} | source={source_lang} | target={target_lang} | {exc}", flush=True)
...
def translation_config_api():
    ...
    try:
        ...
        print(f"🌐 Translation config updated | guild={guild_id} | enabled={cfg['enabled']} | source={cfg['source_channel_id']}", flush=True)
