...
if _original_load_custom_bosses is not None:
    async def safe_load_custom_bosses():
        try:
            ...
            log(f"⚠️ custom_bosses invalid/legacy data skipped: {type(exc).__name__}: {exc}")
        except Exception as exc:
...
async def interaction_diagnostic(interaction):
    try:
        ...
        log(f"📥 INTERACTION RECEIVED | command={command_name!r} user={interaction.user} guild={getattr(interaction.guild, 'id', None)} channel={getattr(interaction, 'channel_id', None)}")
    except Exception as exc:
...
async def startup_heartbeat():
    while True:
        ...
        try:
            log("💓 SKYNET HEARTBEAT | " + f"ready={bot_module.bot.is_ready()} closed={bot_module.bot.is_closed()} user={bot_module.bot.user} guilds={len(bot_module.bot.guilds)}")
...
async def patched_run_bot_with_backoff(token: str):
    # discord.py owns normal Gateway reconnect/resume. This loop handles only
    # failures during initial login/start and never restarts a closed Client
    # without clearing its state first.
    backoff = 30.0
    max_backoff = 300.0
    while True:
        try:
            ...
            if status == 429:
                ...
                log(f"🛑 Discord HTTP 429 / rate limit | retry_after={retry_after:.1f}s | รอ {delay:.1f}s ก่อนเริ่ม session ใหม่")
            else:
                delay = backoff
            ...
            log(f"⚠️ Discord network/timeout error: {type(exc).__name__}: {exc} | retry in {backoff:.1f}s")
            await _reset_client_after_failed_start()
